...
export default async function DashboardPage() {
  ...
  const [subResult, creditsResult, dbResult, licensesResult, eventsResult, txResult] = await Promise.all([
  ...
  const profile = (dbResult as any).data;
  const licenses = licensesResult.data ?? [];
  const billingEvents = eventsResult.data ?? [];
  const creditTransactions = (txResult.data ?? []) as any[];

  const isActive = subscription?.status === "active" || subscription?.status === "trialing";
  const currentPlan = PLANS.find(p => p.id === subscription?.creem_product_id)?.name || "Inactive";
  ...
  if (totalSpentCents === 0 && creditTransactions.length > 0) {
    totalSpentCents = creditTransactions.reduce((sum: number, tx: any) => {
    ...
    <div className="space-y-8 md:space-y-12 pb-12 md:pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-6 border-b pb-6 md:pb-8 text-center md:text-left">
        <div className="flex flex-col items-center md:items-start w-full md:w-auto">
           <span className="text-[10px] font-black uppercase tracking-[0.2em] text-primary mb-1 md:mb-2 block">Account Dashboard</span>
           <h1 className="text-2xl md:text-6xl font-black tracking-tighter uppercase leading-none whitespace-nowrap bg-gradient-to-b from-foreground via-foreground/90 via-80% to-primary/60 bg-clip-text text-transparent">Command Center</h1>
      ...
      <div className="grid gap-4 md:gap-6 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="border border-border shadow-sm bg-card overflow-hidden">
          <CardContent className="p-6 md:p-8">
            ...
            <p className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 opacity-60">Credit Balance</p>
            <h3 className={cn(
              "font-black tracking-tighter uppercase leading-none text-foreground",
              isUnlimited(credits?.balance ?? 0) ? "text-2xl lg:text-3xl" : "text-2xl md:text-4xl"
            )}>
              {isUnlimited(credits?.balance ?? 0) ? "Unlimited" : (credits?.balance ?? 0).toLocaleString()}
            </h3>
          </CardContent>
        </Card>
        ...
        <Card className="border border-border shadow-sm bg-card overflow-hidden text-primary">
           <CardContent className="p-6 md:p-8">
              <div className="flex justify-between items-start mb-4 md:mb-6">
                 <div className={cn("size-10 md:size-12 rounded-xl md:rounded-2xl flex items-center justify-center shadow-inner", isActive ? "bg-success/10 text-success" : "bg-secondary text-muted-foreground")}>
                    <Rocket className="size-5 md:size-6" />
                 </div>
              </div>
              <p className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 opacity-60">Subscription</p>
              <h3 className="text-2xl md:text-4xl font-black tracking-tighter uppercase italic leading-none">{subscription?.status ?? "Offline"}</h3>
           </CardContent>
        </Card>
        ...
        <Card className="border border-border shadow-sm bg-card overflow-hidden text-foreground">
           <CardContent className="p-6 md:p-8">
              ...
              <p className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 opacity-60">Active Licenses</p>
              <h3 className="text-2xl md:text-4xl font-black tracking-tighter uppercase leading-none">{licenses.length}</h3>
           </CardContent>
        </Card>
        ...
        <Card className="border border-border shadow-sm bg-card overflow-hidden text-foreground">
           <CardContent className="p-6 md:p-8">
              ...
              <p className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 opacity-60">Total Spent</p>
               ...
               <CardHeader className="p-6 md:p-8 bg-secondary border-b border-border">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        ...
                        <CardDescription className="text-[10px] uppercase font-black tracking-widest text-primary">Manage your service tier</CardDescription>
                     </div>
                  </div>
               </CardHeader>
               <CardContent className="p-6 md:p-8">
                  {isActive ? (
                     ...
                     <div className="text-center py-12">
                        ...
                        <p className="text-sm font-black uppercase tracking-tight text-muted-foreground mb-6">No active subscription detected.</p>
                        <Button asChild size="lg" className="w-full h-14 rounded-2xl font-black uppercase tracking-tight">
                  ...
                  <CardHeader className="p-6 md:p-8 pb-4">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <CardDescription className="text-[10px] uppercase font-black tracking-widest text-muted-foreground">Manage user seats and access</CardDescription>
                        </div>
                     </div>
                  </CardHeader>
                  <CardContent className="p-6 md:p-8 pt-0">
                     {isActive && subscription?.creem_subscription_id ? (
                        ...
                        <div className="p-6 bg-secondary rounded-[2rem] border-2 border-dashed border-border text-center">
                           <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Locked until subscription starts</p>
                        </div>
                     )}
                  </CardContent>
               </Card>
                   ...
                   <CardHeader className="p-6 md:p-8 pb-4">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <CardDescription className="text-[10px] uppercase font-black tracking-widest text-muted-foreground">Manage your billing preferences</CardDescription>
                        </div>
                     </div>
                  </CardHeader>
                  <CardContent className="p-6 md:p-8 pt-0 space-y-6">
                     <div className="grid sm:grid-cols-2 gap-4">
                        <form action="/api/billing-portal" method="POST" className="h-full">
                           <Button variant="outline" className="w-full h-14 rounded-2xl border-2 font-black uppercase tracking-tight text-[10px] gap-2">
                        ...
                        <div className="h-14 flex items-center px-6 bg-primary rounded-2xl border border-primary/20 shadow-sm">
                           <div className="flex-1">
                              <p className="text-[8px] font-black uppercase tracking-[0.2em] text-primary-foreground/60">Next Payment Date</p>
                              <p className="text-lg font-black uppercase italic text-primary-foreground">
                                 {subscription?.current_period_end ? new Date(subscription.current_period_end).toLocaleDateString() : "Pending"}
                              </p>
                           </div>
                        </div>
                     </div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-1 gap-4">
                  {creditTransactions.slice(0, 8).map((rawTx: any) => {
                    ...
                    return (
                      <div key={tx.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 sm:p-6 rounded-[2rem] bg-background border border-border transition-all hover:bg-card hover:shadow-lg hover:border-primary/20 group gap-4 md:gap-6">
                            ...
                            <div className="min-w-0 flex-1">
                               ...
                               <div className="flex items-center gap-2 mt-1">
                                  <p className="text-[9px] sm:text-[10px] text-muted-foreground font-black uppercase opacity-60">
                                     {tx?.created_at ? new Date(tx.created_at).toLocaleDateString() : "Pending"}
                                  </p>
                         ...
                         <span className={cn("text-lg sm:text-xl font-black tracking-tighter self-end sm:self-center shrink-0", (isUnlimited(tx.amount) || tx.amount > 0) ? "text-success" : "text-destructive")}>
                            {isUnlimited(tx.amount) ? "Unlimited" : `${tx.amount > 0 ? "+" : ""}${tx.amount} Credits`}
                         </span>
                      </div>
                    );
                  })}
                 {creditTransactions.length === 0 && (
                    <div className="text-center py-20 bg-secondary rounded-[3rem] border-2 border-dashed border-border">
                       <p className="text-xs font-black uppercase text-muted-foreground tracking-widest">No history recorded</p>
                    </div>
                 )}
              </div>
           </CardContent>
