import { pgTable, index, text, timestamp, foreignKey, pgPolicy, uuid, integer, bigint, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

// Enums matched from database
export const creditTransactionTypeCheck = pgEnum("credit_transaction_type_check", ['subscription_topup', 'purchase', 'spend', 'refund'])
export const roleCheck = pgEnum("role_check", ['user', 'assistant', 'system'])
export const subscriptionsStatusCheck = pgEnum("subscriptions_status_check", ['active', 'trialing', 'past_due', 'scheduled_cancel', 'cancelled', 'paused', 'expired', 'inactive'])
...
export const billingEvents = pgTable("billing_events", {
	...
	pgPolicy("Users can view own billing events", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
...
export const licenses = pgTable("licenses", {
	...
	pgPolicy("Users can view own licenses", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
...
export const creditTransactions = pgTable("credit_transactions", {
	...
	pgPolicy("Users can view own credit transactions", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
...
export const subscriptions = pgTable("subscriptions", {
	...
	pgPolicy("Users can view own subscription", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
...
export const credits = pgTable("credits", {
	...
	pgPolicy("Users can view own credits", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
...
export const chats = pgTable("chats", {
	...
	pgPolicy("Users can view own chats", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
...
export const purchases = pgTable("purchases", {
	...
	pgPolicy("Users can view own purchases", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
]);
...
export const chatMessages = pgTable("chat_messages", {
	...
	pgPolicy("Users can view own chat messages", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	pgPolicy("Users can create own chat messages", { as: "permissive", for: "insert", to: ["public"] }),
]);
