...
import { Send, Loader2, Bot, User, Sparkles, Trash2, Plus, Menu, X, ChevronRight, MessageSquare, History } from "lucide-react";
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    if (!activeChatId) {
      ...
      setChats(prev => {
        ...
        return [{ ...existing, updated_at: new Date().toISOString() }, ...prev.filter(c => c.id !== activeChatId)];
      });
    }
    ...
    try {
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: "Failed to connect to the system core." }]);
    } finally {
      setLoading(false);
    }
  }
    ...
    <Card className="w-full border-none shadow-2xl bg-card overflow-hidden flex h-[700px] rounded-[2.5rem] relative">
      ...
      <div className={cn(
        ...
        <div className="p-6 border-b flex items-center justify-between">
          <span className="text-[10px] font-black uppercase tracking-widest text-primary">Chat Logs</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
          {chats.map((chat) => (
            ...
            >
              <MessageSquare className={cn("size-4 shrink-0", chatId === chat.id ? "text-white" : "text-slate-400 group-hover:text-primary")} />
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold truncate uppercase tracking-tight">{chat.title}</p>
                <p className={cn("text-[8px] uppercase font-black opacity-60 mt-0.5", chatId === chat.id ? "text-white" : "text-muted-foreground")}>
                  {new Date(chat.updated_at).toLocaleDateString()}
                </p>
              </div>
              <div className="flex items-center shrink-0">
                <ChevronRight className={cn("size-4 opacity-0 group-hover:opacity-100 transition-all", chatId === chat.id && "opacity-100")} />
                <Button
                  variant="ghost"
                  size="icon"
                  className={cn("size-6 p-0 hover:bg-destructive/20 hover:text-destructive opacity-0 group-hover:opacity-100 transition-opacity ml-2", chatId === chat.id && "hover:bg-white/20 text-white")}
        ...
        <CardHeader className="p-4 md:p-8 pb-4 border-b bg-background flex flex-row items-center justify-between shrink-0">
          <div className="flex items-center gap-3 md:gap-4 overflow-hidden">
            ...
            <div className="min-w-0">
              <CardTitle className="text-base md:text-xl font-black uppercase tracking-tighter truncate">AI Core Assistant</CardTitle>
              <CardDescription className="text-[8px] md:text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2 truncate">
                <span className={cn("size-1.5 md:size-2 shrink-0 rounded-full bg-success", initializing ? "animate-spin" : "animate-pulse")} />
                <span className="truncate">{initializing ? "Initializing Memory..." : "Online & Synchronized"}</span>
              </CardDescription>
            </div>
          </div>
        </CardHeader>

        <CardContent className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent" ref={scrollRef}>
          {initializing ? (
            <div className="h-full flex flex-col items-center justify-center space-y-4 opacity-40">
              <Loader2 className="size-8 animate-spin text-primary" />
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Accessing secure memory core...</p>
            </div>
          ) : messages.length === 0 ? (
            ...
            messages.map((m, i) => (
              ...
              >
                <div className={cn(
                  ...
                  m.role === "user" ? "bg-slate-900 text-white" : "bg-primary/10 text-primary border border-primary/20"
                )}>
          ...
          {loading && (
            <div className="flex items-start gap-4 animate-pulse">
              ...
              <div className="p-5 rounded-[1.5rem] bg-secondary border border-border rounded-tl-none">
                <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Synthesizing...</span>
              </div>
            </div>
          )}
        </CardContent>
