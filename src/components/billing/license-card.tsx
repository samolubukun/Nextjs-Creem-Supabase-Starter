...
import { Box, ExternalLink, Key, ShieldCheck } from "lucide-react";
import { Button } from "@/components/ui/button";

interface LicenseCardProps {
  license: {
    product_name?: string | null;
    creem_license_key?: string | null;
    ...
    <div className="p-6 md:p-8 bg-card border border-border rounded-[2.5rem] shadow-sm flex flex-col items-center">
      ...
      <div className="text-center space-y-2 mb-8 w-full">
        <h3 className="text-xl font-black uppercase tracking-tighter text-foreground">
          {license?.product_name || "Enterprise Node"}
        </h3>
      ...
      <div className="w-full space-y-4">
        <div className="p-5 bg-secondary/50 rounded-2xl border border-border">
          ...
          <p className="font-mono text-xs font-bold tracking-tight truncate text-foreground opacity-90">
            {license?.creem_license_key || license?.license_key || "----"}
          </p>
        </div>
        ...
        <div className="pt-2">
          <Button
            variant="outline"
            className="w-full h-11 rounded-xl border-2 font-black uppercase tracking-tight text-[10px] gap-2 transition-all hover:bg-primary hover:text-primary-foreground hover:border-primary"
          >
